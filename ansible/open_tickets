...
$user         = "glpi";
$password     = "glpi";
$xmlhost      = "192.168.1.138";
$xmlurl       = "glpi/plugins/webservices/xmlrpc.php";
$watcher      = "2";
$watchergroup = "1";
$sqlhost      = "192.168.1.138";
$sqldb        = "glpidb";
$sqluser      = "root";
$sqlpwd       = "root";
...
$pega_id_ticket = null;
$num_ticket = null;
$sql = "SELECT id FROM ansible_tickets ORDER BY id DESC LIMIT 1";
$res = mysqli_query($con, $sql);
$pega_id_ticket = mysqli_fetch_array($res);			
$num_ticket = $pega_id_ticket[0] + 1;

if (strlen($num_ticket) === 1) {
    echo "+ Select ID in ansible_tickets --> OK\n";

} else {
    echo "Error: Select ID ansible " . $sqlinsert . "\n" . $con->error;
}
...
if ($event == "OPEN") {
    ...
    if (!empty($session)) {
        $title   = "$state: $servico! - Evento $eventansible gerado automaticamente pelo Ansible";
        ...
        // executa a consulta
        $sqlinsert = "INSERT INTO ansible_tickets (num_event,playbook,event_date)
        VALUES ('$num_ticket', '$servico', current_date)";
        
        if ($con->query($sqlinsert) === TRUE) {
            echo "+ New event created successfully on ansible\n";
        } else {
